...
SELECT --table created for last touch attribution
    UserID,
    Channel,
    Campaign,
    1 AS Conversion_Credit
INTO LastTouchAttribution
FROM (
    SELECT
        UserID,
        Channel,
...
SELECT --table for linear attribution
    UserID,
    Channel,
    Campaign,
    1.0 / COUNT(*) OVER(PARTITION BY UserID) AS Attribution_Credit
INTO LinearAttribution
FROM WebAnalytics;
...
SELECT DISTINCT CAST(EventTimestamp AS DATE)
FROM WebAnalytics;
SELECT MIN(EventTimestamp) AS StartDate,
       MAX(EventTimestamp) AS EndDate
FROM WebAnalytics;
